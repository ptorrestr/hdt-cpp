...
class WaveletSequence : public IntSequence {
...
public:
	WaveletSequence();
	WaveletSequence(IntSequence *otherStream);
	virtual ~WaveletSequence();
	...
	size_t select(size_t symbol, size_t pos);
};
